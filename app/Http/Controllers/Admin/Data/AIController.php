...
- После каждого подзаголовка делайте пустую строку
- После каждого абзаца делайте пустую строку
- Используйте переносы строк для структурирования текста'
        ],
        'gpt-4-turbo-preview' => [
            'max_tokens' => 4000,
            'temperature' => 0.5,
...
- Используйте переносы строк для структурирования текста'
    ...
    private function postProcessText($text) {
        ...
        // Добавляем разделители между событиями
        $text = preg_replace('/🏇|⚾|🏑|⚽|🏀|🎾|🏐|🏈|🏉|🎱|🏓|🏸|🏒|🏑|🏏|🎯|🎳|⛳|🏌️|🏄|🏊|🤽|🚣|🏇|🚴|🚵|🤸|🏋️|🤼|🤾|🏌️|🏄|🏊|🤽|🚣|🏇|🚴|🚵|🤸|🏋️|🤼|🤾/', "\n\n$0", $text);

        // Форматируем заголовки событий
        $text = preg_replace('/\*\*([^*]+)\*\*/', "\n**$1**", $text);

        // Добавляем разделители для результатов и мест проведения
        $text = preg_replace('/(Результат:|Место проведения:)/', "\n$1", $text);

        // Форматируем тире
        $text = preg_replace('/\s*-\s*/', ' — ', $text);

        // Добавляем разделители между событиями (исправленная версия)
        $sportsEmojis = '🏇⚾🏑⚽🏀🎾🏐🏈🏉🎱🏓🏸🏒🏑🏏🎯🎳⛳🏌️🏄🏊🤽🚣🏇🚴🚵🤸🏋️🤼🤾';
        $text = preg_replace('/Место проведения:.*?(?=[' . $sportsEmojis . '])/u', "$0\n\n", $text);
        ...
        // Форматируем запятые в местах проведения
        $text = preg_replace('/,\s*/', ", ", $text);
